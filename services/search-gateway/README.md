...
未配置或调用失败时使用本地关键词过滤（无需额外服务或 Key），并返回降级提示。
中文查询使用 zh-CN、英文使用 en-US，其他语言使用 auto。
结果同时与当前查询和原始问题比对；原始问题中引号内的名称必须出现在标题或摘要中。
每个查询最多保留 5 条，每个主机名最多 2 条，最终最多返回 10 条，过滤后无来源会明确报错。
本地过滤基于英文词和中文双字片段，可能漏掉别名、同义词、跨语言资料；Embedding 可改善语义匹配，阈值仍需结合实际模型验证。
相关性过滤不能修复搜索源的错误结果，也不能用缓存摘要提供精确到分钟的当前时间。
